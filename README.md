...
### Запуск Brelynt Electron

Для работы требуется **Node.js версии 16 или новее**. Чтобы установить зависимости и запустить приложение, выполните:
...
Инструкции по созданию архивного файла `web4.zip` приведены в [release workflow](.github/workflows/release.yml) и в разделе "Creating a release" ниже.
...
В репозитории ранее хранился файл `structure.txt` со списком всех каталогов
проекта, сгенерированный утилитой `tree`. Этот файл не требуется для работы
Web4 и теперь удалён из Git.

Если понадобится снова получить полный список файлов, выполните в корневой
директории проекта:
...
или в Windows-подобной среде:

```cmd
tree /F > structure.txt
```

Храните этот файл локально или вне репозитория, при необходимости добавьте его в
`.gitignore`.
