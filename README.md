...
## Управление P2P узлом

В файле `Brelynt/index.html` есть кнопка **Node ON/OFF**. Она использует
скрипт `p2pBridge.js`, который запускает или останавливает локальный P2P узел.

1. Нажатие кнопки включает соединение с узлами по адресу `ws://localhost:9090`.
2. Повторное нажатие закрывает соединение и останавливает работу узла.

Таким образом, включить или выключить P2P можно прямо из интерфейса или через
консоль браузера вызовом `window.p2pBridge.toggle()`.
